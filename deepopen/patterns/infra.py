...
INFRA_RULES: tuple[PatternRule, ...] = (
    ...
    PatternRule(
        rule_id="SH004",
        title="shell eval 动态执行",
        severity=Severity.HIGH,
        category=Category.SECURITY,
        pattern=_re(r"""(?:^|\s)eval\s+(?:["'$]|\$\()"""),
        message="eval 会把拼接后的字符串当命令执行。",
        remediation="用数组/位置参数展开，避免 eval。",
        cwe="CWE-95",
        suffixes=SHELL,
    ),
    PatternRule(
        rule_id="TF002",
        title="安全组放行 0.0.0.0/0",
        severity=Severity.HIGH,
        category=Category.CONFIG,
        pattern=_re(r"0\.0\.0\.0/0"),
        message="对全网开放入口会把服务暴露到互联网。",
        remediation="把 cidr 收窄到办公网/VPN，或只对 443 配合 WAF。",
        cwe="CWE-284",
        suffixes=TF,
    ),
    PatternRule(
