...
def render_markdown(result: ScanResult) -> str:
    lines = [
        ...
        "",
        f"- 目录: `{result.root}`",
        f"- 文件: {result.files_scanned}",
        f"- 命中: {len(result.findings)}",
        f"- 耗时: {result.duration_ms}ms",
        "",
        "| 级别 | 数量 |",
        "| --- | ---: |",
    ]
    for name, count in result.counts_by_severity.items():
        lines.append(f"| {name} | {count} |")
    lines.append("")
    if not result.findings:
        lines.append("未发现规则命中。这不代表没有缺陷。")
        return "\n".join(lines)
    for finding in result.findings:
        lines.extend(
            [
                ...
                "",
                f"- 位置: `{finding.path}:{finding.line}`",
                f"- 类别: {finding.category.value}" + (f" · {finding.cwe}" if finding.cwe else ""),
                f"- 代码: `{finding.snippet}`",
                f"- 说明: {finding.message}",
                f"- 加固: {finding.remediation}",
                "",
            ]
        )
    return "\n".join(lines)
