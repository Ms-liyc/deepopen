...
本地静态检查工具：扫描你自己的源码，找出常见缺陷、密钥泄露和不安全写法，并给出加固建议。
...
终端扫描 — python -m deepopen scan examples终端扫描 examples

本地控制台 — python -m deepopen serve examples，浏览器打开 http://127.0.0.1:8765/本地 Web 控制台

HTML 报告 — python -m deepopen scan examples -o reports/report.htmlHTML 检查报告

加固清单 — 控制台「清单」页，或 python -m deepopen checklist加固清单
...
deepopen init [--with-ci]



生成 deepopen.toml、忽略文件，可选 GitHub Actions





deepopen hook install
...
-o out.sarif（可上传 GitHub Code Scanning）
...
deepopen/analyzers.py



Dockerfile、依赖清单、.env、Actions 权限





deepopen.toml 的 [[rules]]
